...
def process_money(request):
    ...
    if request.POST['building'] == 'farm':
        ...
        request.session['activities'].append('You earned ' + str(gold) + ' golds from the ' + request.POST['building'] + ' ' + '(' + str(datetime.now().strftime("%Y-%m-%d %H:%M")) + ')')
        ...
        if gold >= 0:
            request.session['activities'].append(' You entered the casino and earned ' + str(gold) +' gold' + ' ' + '(' + str(datetime.now().strftime("%Y-%m-%d %H:%M")) + ')')
        else:
